...
export const make = Effect.fnUntraced(function* (options: {
	...
	const makeRequest = Effect.fnUntraced(function* (providerOptions: LanguageModel.ProviderOptions) {
...
) => Effect.Effect<ReadonlyArray<typeof Generated.Message.Encoded>, AiError.AiError> = Effect.fnUntraced(
	function* (options) {
		...
		for (const message of options.prompt.content) {
			switch (message.role) {
				...
				case "user": {
					...
					if (message.content.length === 1 && firstPart?.type === "text") {
						const cacheControl = getCacheControl(message) ?? getCacheControl(firstPart)
						messages.push({
							role: "user",
							content: Predicate.isNotUndefined(cacheControl)
								? [{ type: "text", text: firstPart.text, cache_control: cacheControl }]
								: firstPart.text,
						})
					} else {
...
) => Effect.Effect<Array<Response.PartEncoded>, AiError.AiError> = Effect.fnUntraced(function* (response) {
...
) => Effect.Effect<Stream.Stream<Response.StreamPartEncoded, AiError.AiError>> = Effect.fnUntraced(
	function* (stream) {
		let idCounter = 0
		...
		return stream.pipe(
			Stream.mapEffect(
				Effect.fnUntraced(function* (event) {
					...
					if (Predicate.isUndefined(choice)) {
						// Usage-only chunk from stream_options.include_usage — no content to emit
						return parts
					}
