...
export function ToolbarSeparator({
	...
	return (
		<ToolbarPrimitive.Separator
			className={cn("mx-2 my-1 w-px shrink-0 bg-border-secondary", className)}
			{...props}
		/>
	)
}
...
const toolbarButtonVariants = cva(
	"inline-flex cursor-pointer items-center justify-center gap-2 rounded-md text-sm font-medium whitespace-nowrap transition-[color,box-shadow] outline-none hover:bg-secondary-hover hover:text-secondary-hover focus-visible:border-brand focus-visible:ring-[3px] focus-visible:ring-brand/50 disabled:pointer-events-none disabled:opacity-50 aria-checked:bg-brand-primary aria-checked:text-primary-on-brand aria-invalid:border-error aria-invalid:ring-error/20 dark:aria-invalid:ring-error/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
	{
		defaultVariants: {
			size: "default",
			variant: "default",
		...
		variants: {
			...
			variant: {
				...
				outline:
					"border border-secondary bg-transparent shadow-xs hover:bg-brand-primary hover:text-primary-on-brand",
			},
		},
	},
)
...
const dropdownArrowVariants = cva(
	cn(
		"inline-flex items-center justify-center rounded-r-md text-sm font-medium text-primary transition-colors disabled:pointer-events-none disabled:opacity-50",
	),
	{
		defaultVariants: {
			size: "sm",
		...
		variants: {
			...
			variant: {
				default:
					"bg-transparent hover:bg-secondary-hover hover:text-secondary-hover aria-checked:bg-brand-primary aria-checked:text-primary-on-brand",
				outline:
					"border border-l-0 border-secondary bg-transparent hover:bg-brand-primary hover:text-primary-on-brand",
			},
		},
	},
)
			...
			>
				{isDropdown ? (
					<>
						...
						<div>
							<ChevronDown className="size-3.5 text-tertiary" data-icon />
						</div>
					</>
				) : (
					children
	...
	return (
		<span
			className={cn(
				...
				"rounded-r-none",
				"group-data-[pressed=true]:bg-brand-primary group-data-[pressed=true]:text-primary-on-brand",
				className,
			)}
			{...props}
		>
