		...
		>
			...
			<AriaSliderTrack className="relative h-6 w-full">
				{({ state: { values, getThumbValue, getThumbPercent, getFormattedValue } }) => {
					...
					return (
						<>
							<span className="-translate-y-1/2 absolute top-1/2 h-2 w-full rounded-full bg-quaternary" />
							<span
								className="-translate-y-1/2 absolute top-1/2 h-2 w-full rounded-full bg-brand-solid"
								style={{
									left: `${left * 100}%`,
									width: `${width * 100}%`,
								}}
